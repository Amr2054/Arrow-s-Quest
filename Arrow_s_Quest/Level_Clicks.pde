// Functions used to manage the clicks in every image according to the mode
void mousePressed() {
  if (mode==STARTMENU) {
    startMenuClicks();
  }
...
void startMenuClicks() {
  ...
  else if(mouseX>1828 && mouseX<1886 && mouseY>1025 && mouseY<1064){
    mode=INSTRUCTION;
    click.play();
}
}
void instructionClicks(){
  if (mouseX>1827 && mouseX<1897 && mouseY>31 && mouseY<73){
  mode=STARTMENU;
  click.play();
  }
}
